...
// The same words `callWasCancelled` in src/domain/close-reasons.jsx reads the
// close reason for. A call called off has no five stamps to give.
const CANCELLED_MARKERS = ["cancel", "stood down", "stand down", "no patient found", "duplicate call", "transported by other means"];
const wasCancelled = (r) => {
  const reason = typeof r.closeReason === "string" ? r.closeReason.trim().toLowerCase() : "";
  return !!reason && CANCELLED_MARKERS.some((m) => reason.includes(m));
};
...
const COMPLETENESS = [
  ...
  // The five stamps, asked of a call that moved somebody — the same rule as
  // the app's, so a timeline gap keeps the record open for the desk to fill.
  ...["enroute", "arrival", "departure", "arrivalDestination", "backInService"].map((k) => ({
    key: `time.${k}`,
    applies: (r) => !r.noTransport && !wasCancelled(r),
    has: (r) => !!((r.times || {})[k]),
  })),
];
...
// Structural, not `JSON.stringify` equality. Two records holding the same data
// with their keys in a different order are the same record, and a comparison
// that says otherwise refuses a write that changed nothing and files a finding
// about it — a wolf cried on the owner's System page.
...
function lockedRecordChange(stored, incoming) {
  ...
  for (const k of keys) {
    ...
    if (!sameValue(stored[k], incoming[k])) return k;
  }
  return null;
}
