...
// The Android twin of pushSound. On Android the sound belongs to the CHANNEL,
// so choosing a tone means choosing which channel the banner arrives on — the
// plugin builds one per tone. Keep these two functions in step: a priority
// that picks the BLS tone on iOS must pick the BLS channel here.
function pushChannel(priority) {
  const p = String(priority || "").toLowerCase();
  return p === "bls" || p === "routine"
    ? "pulseops_dispatch_bls_v1"
    : "pulseops_dispatch_v3";
}

const STAND_DOWN_CHANNEL = "pulseops_stand_down_v1";
...
//
// A message from the desk, a stand-down and a changed destination all reach a
// locked phone — but if they all arrive sounding like a call, a crew learns
// that the alarm tone means "look at your phone" rather than "get up". The
// distinction is the whole value of having a tone at all.
//
//  - a CALL sounds the department's dispatch tone, on the alarm channel;
//  - a STAND-DOWN has its own descending tone: a crew must be able to tell
//    "go" from "stop" without looking;
//  - everything else takes the phone's ordinary notification sound, and on
//    Android goes down NO channel id at all, so it is an ordinary
//    notification rather than something on the alarm stream.
//
// `sound` and `channelId` are therefore both explicit, and a caller that
// passes neither gets a dispatch — which is the only thing that should ever
// be sent without saying what it is.
function callMessage(token, { title, body, data, channelId, priority, sound }) {
  const heading = String(title || "NEW CALL");
  const line = String(body || "");
  const tone = sound || pushSound(priority || (data && data.priority));
  const android = {
    priority: "HIGH",
    notification: {
      title: heading,
      body: line,
      tag: (data && data.kind === "message") ? "dispatch-message" : "dispatch-call",
    },
  };
  // No channel id means Firebase's own fallback channel — an ordinary
  // notification, off the alarm stream. Only a dispatch and a stand-down go
  // down the dispatch channel.
  if (channelId !== null) {
    android.notification.channel_id = String(channelId || pushChannel(priority || (data && data.priority)));
  }
  return {
    message: {
      token,
      ...
      apns: {
        headers: {
          ...
          "apns-collapse-id": (data && data.kind === "message") ? "dispatch-message" : "dispatch-call",
        },
        payload: {
          aps: {
...
async function sendCallAlert(token, { title, body, data, channelId, priority, sound }) {
  const sa = serviceAccount();
  if (!sa) return { ok: false, dead: false };
  const bearer = await accessToken();
  if (!bearer) return { ok: false, dead: false };
  const message = callMessage(token, { title, body, data, channelId, priority, sound });
