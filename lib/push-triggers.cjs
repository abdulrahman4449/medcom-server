...
// The desk's twin of newHandoverAsks: `ems:desk` is one record per station
// (see src/domain/desk-handover.jsx). A NEW pending ask on a held desk wakes
// the holder's phone, once — re-saving the same ask, or answering it, wakes
// nobody.
function newDeskAsks(prevDesk, nextDesk) {
  const pending = (r) => !!(r && r.accountId && r.needsApproval && (!r.status || r.status === "pending"));
  const prev = prevDesk && typeof prevDesk === "object" && !Array.isArray(prevDesk) ? prevDesk : {};
  const next = nextDesk && typeof nextDesk === "object" && !Array.isArray(nextDesk) ? nextDesk : {};
  const out = [];
  for (const station of Object.keys(next)) {
    const rec = next[station];
    if (!rec || typeof rec !== "object") continue;
    const r = rec.relief;
    const holder = rec.holder;
    if (!pending(r) || !holder || !holder.accountId) continue;
    if (String(holder.accountId) === String(r.accountId)) continue;
    const was = prev[station];
    const wr = was && typeof was === "object" ? was.relief : null;
    if (pending(wr) && String(wr.accountId) === String(r.accountId)) continue;
    out.push({ station: String(station), holderAccountId: String(holder.accountId), askerName: String(r.name || r.accountId) });
  }
  return out;
}

module.exports = { newAssignments, newHandoverAsks, newDeskAsks,
